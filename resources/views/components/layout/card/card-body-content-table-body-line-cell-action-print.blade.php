<div class="dropdown float-end" style="width: 40px; padding-top: 6px; padding-left: 5px;">
    <a type="button" data-bs-toggle="dropdown" aria-expanded="false">
        <i class="bi-printer-fill text-primary" style="font-size: 20px;" title="Imprimir relatório"></i>
    </a>
    <ul class="dropdown-menu">
        @php
            $reports = App\Models\Report::where(['folder' => 'price', 'reference_1' => $id])->orderBy('id', 'DESC')->limit(20)->get();
        @endphp
        @foreach($reports as $key => $report)
    <li>
        <a class="dropdown-item" href="{{ asset('storage/pdf/price/' . $report->file) }}" target="_blank">
